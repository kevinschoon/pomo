...
func MatchAny(t Task, filters ...Filter) bool {
	for _, filter := range filters {
		if filter(t) {
			return true
		}
...
func MatchAll(t Task, filters ...Filter) bool {
	for _, filter := range filters {
		if !filter(t) {
			return false
		}
...
func FindMany(tasks []*Task, filters ...Filter) []*Task {
	if len(filters) == 0 {
		return tasks
	}
	var filtered []*Task
	for _, task := range tasks {
		ForEachMutate(task, func(subTask *Task) {
...
func FindOne(tasks []*Task, filters ...Filter) (*Task, error) {
	var (
		result *Task
		err    error
	)
	for _, task := range tasks {
		ForEach(*task, func(subTask Task) {
			if MatchAll(subTask, filters...) {
				if result != nil {
